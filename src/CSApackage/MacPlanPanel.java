...
//      CM_width = K * barZoneWidth;
    ...
    for ( int row = 0; row < numCMRows; row++ )
    {
      ...
      while (col < numCMCols && q < theMac.Q - 1)
      {
        ...
        // paint background diff color for first CM than for the rest.  that background will be same as for
        // the sigmoid graph at upper left.  This tells the user that the values in the sigmoid graph
        // correspond to those in the first CM.  The non-max-whole_sigmoid_V vals in the other CMs are selected randomly from disributions.
